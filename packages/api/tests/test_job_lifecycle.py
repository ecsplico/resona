...
# Inline profile JSON: one replacements step that maps "hello" → "GOODBYE"
_INLINE_PROFILE = json.dumps({
    "name": "test-profile",
    "description": "Test profile for lifecycle test",
    "initial_prompt": [],
    "steps": [
        {
            "type": "replacements",
...
def test_full_job_lifecycle_with_postprocessing(lifecycle_client, clean_db):
    """Submit job with an inline profile, process it, verify transcript and md."""
    import os
    from pathlib import Path

    file_path = Path(os.environ.get("FILE_PATH", "/tmp"))

    # 1. Submit a job with an inline profile that maps "hello" → "GOODBYE"
    wav = _make_wav_bytes()
    resp = lifecycle_client.post(
        "/jobs",
        files=[("audio_files", ("test.wav", wav, "audio/wav"))],
        data={"profile": _INLINE_PROFILE},
    )
    ...
    # 3. Write a dummy audio file so the task can find it
    ...
    # 4. Process the job via the task
    ...
    # 5. Verify job is COMPLETED with correct fields
    resp = lifecycle_client.get(f"/job/{job_id}")
    job = resp.json()
    assert job["status"] == "completed"
    assert job["transcript"] == "hello world"   # raw text from engine
    assert job["md"] == "GOODBYE world"          # postprocessed by the inline profile

    # 6. Verify run_stt was called WITHOUT a replacements kwarg
    mock_run_stt.assert_called_once()
    _, kwargs = mock_run_stt.call_args
    assert "replacements" not in kwargs


def test_job_lifecycle_default_profile(lifecycle_client, clean_db):
    """When no profile is specified, the bundled default profile runs.

    With transcript 'plain text output' (no German dictation keywords), the
    bundled replacements leave the text unchanged, so md == transcript.
    """
    import os
    from pathlib import Path
