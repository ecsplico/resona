"""Tests for resona_client.config — BackendConfig, BackendEntry, is_reachable, resolve_backend."""
import json
import pytest
import httpx
import respx
...
from resona_client.config import BackendConfig, BackendEntry, is_reachable, resolve_backend
...
def test_load_migrates_from_legacy_config(tmp_path, monkeypatch):
    ...
    cfg = BackendConfig.load()
    assert len(cfg.backends) == 1
    assert cfg.backends[0].name == "migrated"
...
def test_load_does_not_migrate_when_resona_config_exists(tmp_path, monkeypatch):
    ...
    resona_config.write_text(json.dumps({
        "backends": [{"name": "current", "api_url": "http://current:7000", "api_key": "", "compose_dir": None}]
    }))
    legacy_config = tmp_path / ".whisper-server" / "config.json"
    legacy_config.parent.mkdir(parents=True)
    legacy_config.write_text(json.dumps({
        "backends": [{"name": "legacy", "api_url": "http://legacy:7000", "api_key": "", "compose_dir": None}]
    }))
...
def test_resolve_backend_returns_first_reachable(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("resona_client.config.BackendConfig.load", lambda: BackendConfig(backends=[entry]))
...
def test_resolve_backend_skips_unreachable_tries_next(monkeypatch):
    ...
    monkeypatch.setattr("resona_client.config.BackendConfig.load",
