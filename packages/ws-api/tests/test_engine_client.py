"""Tests for ws_api.engine_client.EngineClient using respx."""
import io
import json
import struct
import tempfile
...
def test_transcribe_sends_replacements_as_json(engine_client, audio_file):
    replacements = [{"name": "foo", "replacement": "bar"}]
    with respx.mock:
        route = respx.post(f"{ENGINE_URL}/transcribe").mock(
            return_value=httpx.Response(200, json={"text": "", "language": "de", "segments": []})
        )
        engine_client.transcribe(audio_file, replacements=replacements)
    request = route.calls.last.request
    body = request.content.decode('latin-1')
    assert json.dumps(replacements) in body


def test_transcribe_no_replacements_when_none(engine_client, audio_file):
    with respx.mock:
        route = respx.post(f"{ENGINE_URL}/transcribe").mock(
            return_value=httpx.Response(200, json={"text": "", "language": "de", "segments": []})
        )
        engine_client.transcribe(audio_file, replacements=None)
