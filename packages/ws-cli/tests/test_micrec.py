"""Tests for ws_cli.micrec — RecordingSession and MicRecApp (no real audio hardware)."""
import queue
import threading
...
from ws_cli.micrec import RecordingSession, MicRecApp
...
def test_run_mic_rec_app_audio_error_exits(tmp_path, monkeypatch):
    ...
    with (
        patch("ws_cli.micrec.sd.check_input_settings", side_effect=Exception("no mic")),
        pytest.raises(SystemExit),
    ):
        from ws_cli.micrec import run_mic_rec_app
        run_mic_rec_app()
