# apps/resona-cli/tests/test_local_engine.py
...
def test_find_free_port_returns_integer():
    from resona_cli.local_engine import _find_free_port
...
def test_enter_spawns_subprocess_with_port_env(tmp_path):
    ...
    assert "RESONA_ENGINE_KEY" not in env_used
    engine.__exit__(None, None, None)


def test_enter_spawns_resona_engine_command(tmp_path):
    mock_proc = _make_mock_process()
    mock_http = _make_healthy_http_client()
    ...
    assert cmd_used == ["uv", "run", "resona-engine-faster-whisper"]
    engine.__exit__(None, None, None)
...
def test_enter_injects_model_override(tmp_path):
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54322),
        patch("subprocess.Popen", return_value=mock_proc) as mock_popen,
        patch("httpx.Client", return_value=mock_http),
        patch("time.sleep"),
    ):
...
def test_enter_raises_if_process_dies_before_health():
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54323),
...
def test_enter_raises_on_health_timeout():
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54324),
...
def test_transcribe_posts_to_correct_url(tmp_path):
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54325),
...
def test_transcribe_sends_language(tmp_path):
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54326),
        patch("subprocess.Popen", return_value=mock_proc),
        patch("httpx.Client", return_value=mock_http),
        patch("time.sleep"),
    ):
...
def test_exit_terminates_process():
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54327),
        patch("subprocess.Popen", return_value=mock_proc),
        patch("httpx.Client", return_value=mock_http),
        patch("time.sleep"),
    ):
...
def test_exit_kills_if_terminate_hangs():
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54328),
        patch("subprocess.Popen", return_value=mock_proc),
        patch("httpx.Client", return_value=mock_http),
        patch("time.sleep"),
    ):
...
def test_enter_cleans_up_resources_on_health_failure():
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54330),
        patch("subprocess.Popen", return_value=dead_proc),
        patch("httpx.Client", return_value=mock_http),
        patch("tempfile.TemporaryFile") as mock_tf,
        patch("time.sleep"),
...
def test_atexit_handler_unregistered_after_clean_exit():
    ...
    with (
        patch("resona_cli.local_engine._find_free_port", return_value=54329),
        patch("subprocess.Popen", return_value=mock_proc),
        patch("httpx.Client", return_value=mock_http),
        patch("time.sleep"),
