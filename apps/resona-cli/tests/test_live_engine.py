...
def test_explicit_engine_when_installed(monkeypatch):
    monkeypatch.setattr(reg, "installed_engines", lambda: ["faster-whisper", "mlx-whisper"])
...
def test_defaults_to_platform_preferred(monkeypatch):
    monkeypatch.setattr(reg, "installed_engines", lambda: ["faster-whisper", "mlx-whisper"])
    monkeypatch.setattr(reg, "recommended_engine", lambda: "mlx-whisper")
    assert _resolve_live_engine(None) == "mlx-whisper"


def test_respects_existing_env(monkeypatch):
    monkeypatch.setattr(reg, "installed_engines", lambda: ["faster-whisper", "whisper"])
...
def test_uninstalled_engine_exits(monkeypatch):
    monkeypatch.setattr(reg, "installed_engines", lambda: ["faster-whisper"])
