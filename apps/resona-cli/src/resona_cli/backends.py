"""CLI commands for managing resona backends."""
from typing import Optional

import typer

from resona_client.config import BackendConfig, BackendEntry, is_reachable
...
def list_backends():
    ...
    if not cfg.backends:
        ...
        typer.echo("  Add one with:  resona backends add <name> <url>")
        return
...
def add_backend(
    ...
    Examples:
      ...
      resona backends add remote http://localhost:17000 --ssh user@myserver.com --ssh-remote-port 7000
    """
    cfg = BackendConfig.load()
    entry = BackendEntry(
        name=name,
