"""Tests for ws_cli.backends CLI commands."""
import json
...
def patch_config(tmp_path):
    ...
    return (
        patch("ws_client.config.CONFIG_DIR", tmp_path),
        patch("ws_client.config.CONFIG_FILE", config_file),
    )
...
def test_list_no_backends(tmp_path):
    with patch("ws_client.config.CONFIG_FILE", tmp_path / "config.json"):
...
def test_list_shows_backends(tmp_path):
    from ws_client.config import BackendConfig, BackendEntry
    ...
    with (
        patch("ws_client.config.CONFIG_FILE", config_file),
        patch("ws_cli.backends.is_reachable", return_value=False),
    ):
        result = runner.invoke(app, ["backends", "list"])
...
def test_add_backend_with_key(tmp_path):
    ...
    with (
        patch("ws_client.config.CONFIG_DIR", tmp_path),
        patch("ws_client.config.CONFIG_FILE", config_file),
        patch("ws_cli.backends.is_reachable", return_value=True),
    ):
...
def test_test_specific_backend_not_found(tmp_path):
    ...
    with patch("ws_client.config.CONFIG_FILE", config_file):
