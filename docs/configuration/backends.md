...
`ws-client` (and all tools built on it) supports multiple backend servers with automatic fallback, SSH tunnelling, and Docker auto-start.

## Configuration file

Backends are stored in `~/.whisper-server/config.json`:
  ...
  "backends": [
    {
      ...
      "api_key": "",
      "compose_dir": "/home/user/whisper-server",
      "ssh_host": null,
...
Manage with `ws-cli backends`:

```bash
ws-cli backends list          # show all + reachability
ws-cli backends add ...       # add new entry
ws-cli backends remove <name> # delete entry
ws-cli backends test [name]   # probe health endpoint
```

## Resolution order

When `WhisperClient.from_config()` is called:

1. **`WS_API_URL` env var** — if set, used directly; config file is ignored.
...
```bash
ws-cli backends add lan http://192.168.1.10:7000 --key mysecret
```

### Docker Compose auto-start

Set `--compose-dir` to the project root. When the backend is unreachable, ws-client runs `docker compose up -d` in that directory and waits for the service to become healthy.
...
# Skip auto-start
client = WhisperClient.from_config(auto_start=False)
```
